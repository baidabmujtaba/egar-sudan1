...
import { MapPin, Phone, MessageCircle, Bed, Video, GraduationCap, UtensilsCrossed } from "lucide-react";
...
export default function DormitoryCard({
  title, price, location, gender, available_spots, beds_per_room,
  meals_included, nearby_university, phone_number, video_url,
}: DormitoryCardProps) {
  const { user } = useAuth();
  ...
  const requireLogin = (e: React.MouseEvent) => {
    if (!user || !phone_number) { e.preventDefault(); navigate("/login"); return true; }
    return false;
  };
      ...
      <CardContent className="p-5 space-y-4">
        ...
        <div className="flex flex-wrap gap-2">
          <div className="flex items-center gap-1.5 text-xs bg-muted/60 rounded-full px-3 py-1.5">
            ...
            <span>{beds_per_room} أسرّة/غرفة</span>
          </div>
          <div className="flex items-center gap-1.5 text-xs bg-muted/60 rounded-full px-3 py-1.5">
            <Bed className="h-3.5 w-3.5 text-primary" />
            <span>{available_spots} متاح</span>
          </div>
          <div className="flex items-center gap-1.5 text-xs bg-muted/60 rounded-full px-3 py-1.5">
